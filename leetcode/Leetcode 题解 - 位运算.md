<!-- GFM-TOC -->
* [0. 原理](#0-原理) 
* [461. 汉明距离](#1-统计两个数的二进制表示有多少位不同) easy not ac
* [136. 数组中唯一一个不重复的元素](#2-数组中唯一一个不重复的元素) easy not ac
* [268. 找出数组中缺失的那个数](#3-找出数组中缺失的那个数) easy not ac
* [260. 数组中不重复的两个元素](#4-数组中不重复的两个元素) medium not ac
* [190. 翻转一个数的比特位](#5-翻转一个数的比特位) easy not ac
* [6. 不用额外变量交换两个整数](#6-不用额外变量交换两个整数)  ac
* [231. 判断一个数是不是 2 的 n 次方](#7-判断一个数是不是-2-的-n-次方) easy not ac
* [342.  判断一个数是不是 4 的 n 次方](#8--判断一个数是不是-4-的-n-次方) easy not ac
* [693. 判断一个数的位级表示是否不会出现连续的 0 和 1](#9-判断一个数的位级表示是否不会出现连续的-0-和-1) easy not ac
* [476. 求一个数的补码](#10-求一个数的补码) easy not ac
* [371. 实现整数的加法](#11-实现整数的加法) easy not ac
* [318. 字符串数组最大乘积](#12-字符串数组最大乘积) medium not ac 
* [338. 统计从 0 \~ n 每个数的二进制表示中 1 的个数](#13-统计从-0-\~-n-每个数的二进制表示中-1-的个数) medium not ac
<!-- GFM-TOC -->
