...
* [斐波那契数列](#斐波那契数列)
    * [70. 爬楼梯](#1-爬楼梯)  easy
    * [198. 强盗抢劫](#2-强盗抢劫) easy 
    * [213. 强盗在环形街区抢劫](#3-强盗在环形街区抢劫)  medium not ac
    * [4. 信件错排](#4-信件错排)   
    * [5. 母牛生产](#5-母牛生产)
* [矩阵路径](#矩阵路径)
    * [64. 矩阵的最小路径和](#1-矩阵的最小路径和) medium ac
    * [62. 矩阵的总路径数](#2-矩阵的总路径数) medium not ac
* [数组区间](#数组区间)
    * [303. 数组区间和](#1-数组区间和) easy not ac
    * [413. 数组中等差递增子区间的个数](#2-数组中等差递增子区间的个数) medium  not ac
* [分割整数](#分割整数)
    * [343. 分割整数的最大乘积](#1-分割整数的最大乘积) medium not ac
    * [279. 按平方数来分割整数](#2-按平方数来分割整数) medium not ac
    * [91. 分割整数构成字母字符串](#3-分割整数构成字母字符串) medium not ac
* [最长递增子序列](#最长递增子序列)
    * [300. 最长递增子序列](#1-最长递增子序列)  LIS  medium not ac
    * [646. 一组整数对能够构成的最长链](#2-一组整数对能够构成的最长链) medium not ac  贪心
    * [376. 最长摆动子序列](#3-最长摆动子序列) medium not ac
* [最长公共子序列](#最长公共子序列) 
    * [1143. 最长公共子序列](#1-最长公共子序列)  LCS medium not ac
* [0-1 背包](#0-1-背包)
    * [416. 划分数组为和相等的两部分](#1-划分数组为和相等的两部分) medium not ac
    * [494. 改变一组数的正负号使得它们的和为一给定数](#2-改变一组数的正负号使得它们的和为一给定数) medium not ac
    * [474. 01 字符构成最多的字符串](#3-01-字符构成最多的字符串)
...
public boolean canPartition(int[] nums) {
    ...
    for (int num : nums) {                 // 0-1 背包一个物品只能用一次
        for (int i = W; i >= num; i--) {   // 从后往前，先计算 dp[i] 再计算 dp[i-num]
            dp[i] = dp[i] || dp[i - num];  //背包问题状态转移方程 v[i,w] = max(v[i-1,w], v[i-1,w-wi]) 包括与不包括itemi
        }
    }
    return dp[W];
}
